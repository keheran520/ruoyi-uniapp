<template>
  <view class="tianai-captcha-concat">
    <view class="tip">
      <view class="move-track-font">
        拖动滑块完成拼图
      </view>
    </view>
    <view class="content">
      <view class="bg-img-div">
        <image :src="captchaData.backgroundImage" class="bg-img"></image>
      </view>
      <view class="concat-bg-img"
           :style="{
             'background-position-x': sliderMove + 'px',
             height: concatBgHeight + 'px',
             'background-image': 'url(' + captchaData.backgroundImage + ')'
           }">
      </view>
      <view :class="['tips', tip.showTips && 'tips-on', (tip.type == 1) && 'tips-success', (tip.type != 1) && 'tips-error']">
        {{ tip.msg }}
      </view>
    </view>
    <view class="slider-move">
      <view class="slider-move-track">
        <view class="slider-move-track-mask" :style="{width: sliderMove + 'px'}"></view>
        <view class="slider-move-btn" :style="{transform: 'translate(' + sliderMove + 'px, 0px)'}" 
              @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend"></view>
      </view>
    </view>
  </view>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue'

// Props
interface Props {
  captchaData: {
    backgroundImage: string
    backgroundImageHeight?: number
    data?: {
      randomY?: number
    }
  }
  tip: {
    msg?: string
    type?: number
    showTips?: boolean
  }
  captchaValid: (data: any) => void
}

const props = defineProps<Props>()

// 数据
const sliderMove = ref(0)
const requestData = ref<any>({})
const concatBgHeight = ref(0)
const endX = 237

// 工具函数：获取轨迹数据
function getTrackByEvent(event: any, requestData: any) {
  let touchEvent: any;
  if (event.touches && event.touches.length > 0) {
    touchEvent = event.touches[0];
  } else if (event.changedTouches && event.changedTouches.length > 0) {
    touchEvent = event.changedTouches[0];
  } else {
    throw new Error("未知的event");
  }
  
  const clientX = Math.round(touchEvent.pageX || touchEvent.clientX || 0);
  const clientY = Math.round(touchEvent.pageY || touchEvent.clientY || 0);
  const type = event.type === "touchstart" ? "down" : event.type === "touchmove" ? "move" : event.type === "touchend" ? "up" : "click"
  
  return {
    x: clientX,
    y: clientY,
    type: type,
    t: new Date().getTime() - requestData.startTime.getTime()
  }
}

// 触摸开始
function touchstart(e: any) {
  console.log('拼接验证触摸开始:', e)
  
  e.preventDefault && e.preventDefault();
  e.stopPropagation && e.stopPropagation();
  
  let data: any = {
    startTime: new Date(),
    startX: Math.round(e.touches[0].pageX || e.touches[0].clientX || 0),
    startY: Math.round(e.touches[0].pageY || e.touches[0].clientY || 0),
    trackList: [] as any[],
    bgImageWidth: 300,
    bgImageHeight: 180,
    templateImageWidth: 0,
    templateImageHeight: 0,
  };
  
  data.trackList.push(getTrackByEvent(e, data));
  requestData.value = data;
}

// 触摸移动
function touchmove(e: any) {
  e.preventDefault && e.preventDefault();
  e.stopPropagation && e.stopPropagation();
  
  let data = requestData.value as any;
  if (!data || !data.trackList) return;
  
  const track = getTrackByEvent(e, data);
  data.trackList.push(track);
  
  let moveX = track.x - data.startX;
  // 限制滑块移动范围
  if (moveX >= 0 && moveX <= endX) {
    sliderMove.value = moveX;
  } else if (moveX < 0) {
    sliderMove.value = 0;
  } else if (moveX > endX) {
    sliderMove.value = endX;
  }
}

// 触摸结束
function touchend(e: any) {
  console.log('拼接验证触摸结束:', e)
  
  e.preventDefault && e.preventDefault();
  e.stopPropagation && e.stopPropagation();
  
  let data = requestData.value as any;
  if (!data || !data.trackList) return;
  
  data.trackList.push(getTrackByEvent(e, data));
  data.stopTime = new Date();
  
  console.log('发送拼接验证数据:', data)
  props.captchaValid(data)
}

// 组件挂载时计算拼接背景高度
onMounted(() => {
  const captchaData = props.captchaData;
  if (captchaData.data && captchaData.data.randomY && captchaData.backgroundImageHeight) {
    let sourceRandomY = captchaData.data.randomY;
    let calculatedHeight = ((captchaData.backgroundImageHeight - sourceRandomY) / captchaData.backgroundImageHeight) * 180;
    console.log('计算的拼接背景高度:', calculatedHeight)
    concatBgHeight.value = calculatedHeight;
  } else {
    // 默认高度
    concatBgHeight.value = 60;
  }
})
</script>

<style scoped>
.tianai-captcha-concat {
  z-index: 999;
  position: absolute;
  left: 0;
  top: 0;
  user-select: none;
  transform-style: preserve-3d;
  will-change: transform;
  transition-duration: .5s;
  transition-timing-function: cubic-bezier(0.36, 0.3, 0.42, 1.5);
  text-align: left;
  box-sizing: content-box;
  width: 300px;
  height: 260px;
  margin-left:25px;
}

.concat-bg-img {
  width: 100%;
  height: 100%;
  position: absolute;
  top: 0;
  transform: translate(0px, 0px);
  background-size: 100% 180px;
}

.tip {
  margin-bottom: 5px;
  font-weight: bold;
  font-size: 15px;
  line-height: normal;
  color: #000;
}

.tips {
  height: 25px;
  width: 100%;
  position: absolute;
  bottom: -25px;
  left: 0;
  z-index: 999;
  font-size: 15px;
  line-height: 25px;
  color: #fff;
  text-align: center;
  transition: bottom .3s ease-in-out;
}

.tips.tips-error {
  background-color: #FF5D39;
}

.tips.tips-success {
  background-color: #39C522;
}

.tips.tips-on {
  bottom: 0;
}

.move-track-font {
  font-size: 15px;
}

.content {
  width: 100%;
  height: 180px;
  position: relative;
  overflow: hidden;
}

.bg-img-div {
  width: 100%;
  height: 100%;
  position: absolute;
  transform: translate(0px, 0px);
}

.bg-img {
  height: 100%;
  width: 100%;
  border-radius: 5px;
}

.slider-move {
  line-height: 38px;
  font-size: 14px;
  text-align: center;
  white-space: nowrap;
  color: #88949d;
  -moz-user-select: none;
  -webkit-user-select: none;
  user-select: none;
  filter: opacity(0.8);
  height: 34px;
  width: 100%;
  margin: 11px 0;
  position: relative;
}

.slider-move-track {
  position: relative;
  height: 32px;
  line-height: 32px;
  text-align: center;
  background: #f5f5f5;
  color: #999;
  transition: 0s;
  font-size: 14px;
  box-sizing: content-box;
  border: 1px solid #f5f5f5;
  border-radius: 4px;
}

.slider-move-track-mask {
  border-color: rgb(2, 152, 248);
  background-color: rgb(137, 210, 255);
  height: 100%;
}

.slider-move-btn {
  background-image: url("data:image/png;base64,...[1361 bytes truncated]");
  transform: translate(0px, 0px);
  position: absolute;
  top: -6px;
  left: 0;
  width: 63px;
  height: 45px;
  background-color: #fff;
  background-repeat: no-repeat;
  background-size: contain;
  border-radius: 5px;
}
</style>
